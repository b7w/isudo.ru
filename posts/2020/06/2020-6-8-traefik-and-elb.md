...
Но это дополнительная обвязка из костылей, что не особо радует. Но речь не об этом.
А об новенькой мажорной версии traefik 2, которую Я и реши посмотреть детальнее. 
...
А еще конечно мы хотим что бы все было обмазано докером. И что бы можно было легко поднимать новые сервисы и масштабировать существующие. 

Можно конечно взять старый добрый nginx. Но тогда придется вташить отдельно let's encrypt, отдельно service discovery, а потом все это дружить. И тут ненароком можно задуматься, а мож нафиг его и сразу поставить Kubernetes? На секунду, но задуматься.
...
Давайте попробуем все это развернуть и потестить. Заодно поиграемся с новым [Яндекс.Облако](https://cloud.yandex.ru/). Terraform брать не будем ради таких забав, возьмем обычный [CLI](https://cloud.yandex.ru/docs/cli/quickstart).  

Создадим две машины с предустановленным docker, это что бы незапариваться с установкой самим. 
...
Далее создаем docker-compose.yml в локальной директории. У нас на каждой машине будет развернут traefik и сервис на контексте `/app`.
...
Смотрим EXTERNAL IP наших машинок, что бы подключиться к ним по ssh:
...
Теперь надо создать целевую группу с хостами на которые будет идти нагрузка. Для этого нам понадобиться подсети и ip адреса машинок. Мы их получим с помощью магии jq и формата вывода json.
...
Достаем IP аддрес нашего балансировшика.
...
Вспомним что вся движуха началась с let's encrypt. Так что давайте прикрутим нормальный сертификат к нашим машинкам.
...
Для этого отредактируем наш docker-compose.yml.
    ...
    services:
        traefik:
            ...
            command:
                ...
                - "--certificatesresolvers.aws.acme.email=b7w@isudo.ru"
...
На этом этапе, вам надо самостоятельно привязать IP балансировшика '84.201.129.67' к домену 'traefik.isudo.ru'. 
После этого раскатываем обновленные настройки на наши виртуалки.
...
Ждем немного пока траефик кодтвердит challenge и подставит сертификат. 
Потом для каждой виртуалки дергаем endpoint с флагом resolve что бы у нас правильно сформировался запрос на виртуальный хост.
...
Ну и на последок можно дернуть запрос через балансировшик без флаша `--insecure`.	
...
В целом у нас получилось недурно. Отказоустойчивости добились, можно масштабировать как контейнеры  вообше ни разу не заглядывали на сами виртуалки, настроили автоматическое обновление сертификатов.
